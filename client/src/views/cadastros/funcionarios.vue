<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.find.nome" placeholder="Nome" style="width: 300px;" class="filter-item" @keyup.enter.native="handleFilter" />
...
export default {
  ...
  methods: {
    getList() {
      ...
      const self = this
      fetchList('funcionarios', this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          create('funcionarios', this.temp).then((ret) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          update('funcionarios', tempData).then(() => {
    ...
    handleDelete(row) {
        this.$confirm('Excluir registro?', 'Aviso', {
          ...
          type: 'warning'
            ...
            // Server order
            deleteItem('funcionarios', row).then(() => {
