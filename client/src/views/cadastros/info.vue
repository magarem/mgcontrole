  ...
  methods: {
    ...
    getList() {
      ...
      fetchList('info', this.listQuery).then(response => {
        ...
        // this.preferences = this.list[0].preferences
        this.total = response.data.total
        console.log('this.list[0]>>>:', this.list[0])
        var d = this.list[0]
        self.preferences = JSON.parse(d.preferences)
        console.log('d.preferences:', self.preferences)
        this.handleUpdate(d)
      })
    },
    resetTemp() {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          var tempData = {...this.temp, preferences: JSON.stringify(this.preferences)}
