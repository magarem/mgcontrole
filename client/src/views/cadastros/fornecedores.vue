<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.find.razao_social" placeholder="Razão social /  Nome fantasia" style="width: 300px;" class="filter-item" @keyup.enter.native="handleFilter" />
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchList('fornecedores', this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          create('fornecedores', this.temp).then(() => {
    ...
    deleteData() {
      delete ('fornecedores', this.temp).then(() => {
        this.list.unshift(this.temp)
        this.dialogFormVisible = false
        this.$notify({
          title: 'Sucesso',
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          update('fornecedores', tempData).then(() => {
    ...
    handleDelete(row) {
        this.$confirm('Excluir registro?', 'Aviso', {
          ...
          type: 'warning'
            ...
            // Server order
            deleteItem('fornecedores', row).then(() => {
