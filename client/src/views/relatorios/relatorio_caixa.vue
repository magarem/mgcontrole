...
<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column label="Data" prop="data"  width="280">
        <template slot-scope="scope">
          <span>{{ scope.row.created | data}}</span>
        </template>
      </el-table-column>

      <el-table-column label="Sessão"   width="350">
        <template slot-scope="scope">
          <span>{{ scope.row.session }}</span>
        </template>
      </el-table-column>
     <el-table-column label="Operação"  width="140">
        <template slot-scope="scope">
          <span>{{ scope.row.op | caixa_op_filter}}</span>
        </template>
      </el-table-column>
      <el-table-column label="Valor"  width="600">
        <template slot-scope="scope">
          <span>{{ scope.row.value}}</span>
        </template>
      </el-table-column>
       <el-table-column label="Obs"  width="600">
        <template slot-scope="scope">
...
export default {
  ...
  filters: {
    ...
    caixa_op_filter(op){
        ...
        if (op=='sangria') return 'Sangria'
        if (op=='reforco') return 'Reforço'
    },
    decimal(value) {
      if (!value) return '0'
  ...
  methods: {
    ...
    getList() {
      ...
      const self = this
      fetchList('caixa_status', this.listQuery).then(response => {
